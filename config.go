...
func WithSSMClient(c GetParametersAPIClient) LoadOptFunc {
...
// Load loads configuration values from AWS SSM Parameter Store and environment variables into a struct.
// The struct must be passed as a pointer.
// The struct fields must be tagged with `ssm` or `env` to indicate the source of the value.
// The `prefix` option can be used to specify a prefix for SSM parameter names.
// The `client` option can be used to specify an SSM client to use. If not provided, the default client will be used.
// Returns an error if the struct is not a pointer to a struct, if a required value is missing, or if there is an error fetching values from SSM.
